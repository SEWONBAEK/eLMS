...
function InfoUpdate() {
	...
	$.ajax({
		...
		success : function(data)
		{
			data = data.trim();
			alert("회원정보가 수정되었습니다.");
			document.location = "../apply/apply.do";
		},
                ...
                <div class="edit-card">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <c:choose>
									<c:when test="${major == '01'}"><c:set var="gradeVal" value="컴퓨터공학과"/></c:when>
									<c:when test="${major == '02'}"><c:set var="gradeVal" value="경영학과"/></c:when>
									<c:when test="${major == '03'}"><c:set var="gradeVal" value="국어국문학과"/></c:when>
									<c:when test="${major == '04'}"><c:set var="gradeVal" value="기계공학과"/></c:when>
									<c:when test="${major == '05'}"><c:set var="gradeVal" value="사회학과"/></c:when>
									<c:when test="${major == '06'}"><c:set var="gradeVal" value="시각디자인학과"/></c:when>
									<c:when test="${major == '07'}"><c:set var="gradeVal" value="심리학과"/></c:when>
									<c:when test="${major == '08'}"><c:set var="gradeVal" value="생명공학과"/></c:when>
									<c:when test="${major == '09'}"><c:set var="gradeVal" value="전자공학과"/></c:when>
									<c:when test="${major == '10'}"><c:set var="gradeVal" value="정치외교학과"/></c:when>
									<c:otherwise>컴퓨터공학과</c:otherwise>
							</c:choose>
                            <input type="text" class="form-control custom-input" name="grade" value="${gradeVal}" readonly>
                        </div>
                    </div>
                    <div class="form-footer">
                    	<!-- 정보수정완료시 alert로 변경완료 알림 -->
                        <button class="btn-upsubmit" onclick="InfoUpdate();">정보 수정 완료</button>
                        <a href="../apply/apply.do"><button class="btn-upcancel">취소</button></a>
                    </div>
                </div>
            </div>
        </main>
